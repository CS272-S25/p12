document.addEventListener("DOMContentLoaded", () => {
    ...
    async function displayTeamWithStats(team) {
      ...
      const url = `https://www.balldontlie.io/api/v1/season_averages?season=${season}&${query}`;
    ...
    if (saved) {
      ...
      // Wait until playerMap is ready before populating
      const checkReady = setInterval(() => {
        if (Object.keys(playerMap).length > 0) {
          positions.forEach(pos => {
            document.getElementById(pos).value = team[pos.toUpperCase()];
          });
          displayTeamWithStats(team);
